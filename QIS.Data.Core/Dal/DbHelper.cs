...
EXEC('SELECT IDENTITY(decimal('+@l_maxprec+',0),'+@l_IdentitySave+',1) id INTO #tmp')";
        ...
        public string GetRowIndex(string filterExpression, string keyField, string keyValue, string orderByExpression)
        {
            ...
            return string.Format("SELECT a.row FROM (SELECT ROW_NUMBER() OVER (ORDER BY {0}) - 1 as row, {2} FROM {1}{4}) a WHERE {2} LIKE '%{3}%'", orderByExpression, _tableName, keyField, keyValue, filterExpression);
        }
        ...
        public string SelectMaxColumn(string columnName)
        {
            string result = string.Format("SELECT TOP(1) {0} FROM {1} ORDER BY {0} DESC", columnName, _tableName);
            return result;
        }
