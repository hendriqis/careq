...
<script type="text/javascript" id="dxss_serviceunithealthcareentryctl">
    $('#lblEntryPopupAddData').live('click', function () {
        ...
        $('#<%=chkIsSpecialPrice.ClientID %>').prop('checked', false);
        $('#<%=txtPrice.ClientID %>').val('0').trigger('changeValue');
        $('#<%=chkIsFinalDiscount.ClientID %>').prop('checked', false);
        $('#<%=txtDiscountAmount.ClientID %>').val('0').trigger('changeValue');

        $('#<%=chkIsSpecialPrice.ClientID %>').change();

        $('#containerPopupEntryData').show();
    });

    $('#<%=chkIsSpecialPrice.ClientID %>').change(function () {
        if (!$(this).is(':checked')) {
            $('#<%=chkIsFinalDiscount.ClientID %>').attr("disabled", true);
            ...
            $('#<%=chkIsFinalDiscount.ClientID %>').removeAttr("disabled");
    ...
    $('.imgEdit.imgLink').live('click', function () {
        ...
        var price = parseFloat($row.find('.hdnPrice').val());
        var discountAmount = parseFloat($row.find('.hdnDiscountAmount').val());
        var isFinalDiscountInPercentage = $row.find('.hdnIsFinalDiscountInPercentage').val() == '1';
        ...
        $('#<%=chkIsSpecialPrice.ClientID %>').prop('checked', price != 0);
        $('#<%=txtPrice.ClientID %>').val(price).trigger('changeValue');
        $('#<%=chkIsFinalDiscount.ClientID %>').prop('checked', isFinalDiscountInPercentage);
                ...
                <div id="containerPopupEntryData" style="margin-top:10px;display:none;">
                    ...
                    <fieldset id="fsEntryPopup" style="margin:0"> 
                        <table class="tblEntryDetail" style="width:100%">
                            ...
                            <tr>
                                <td>&nbsp;</td>
                                <td><asp:CheckBox ID="chkIsSpecialPrice" runat="server" /><%=GetLabel("Special Price") %></td>
                            </tr>
                            <tr>
                                <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Price")%></label></td>
                                <td><asp:TextBox ID="txtPrice" CssClass="txtCurrency" Width="120px" runat="server" /></td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                                <td><asp:CheckBox ID="chkIsFinalDiscount" runat="server" /><%=GetLabel("Final Discount") %></td>
                            </tr>
                            <tr>
                                <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Discount Amount")%></label></td>
                                <td><asp:TextBox ID="txtDiscountAmount" CssClass="txtCurrency" Width="120px" runat="server" /></td>
                            </tr>
                            <tr>
                                <td colspan="2">
                                    <table>
                                        ...
                                        <Columns>
                                            <asp:TemplateField HeaderStyle-Width="70px" ItemStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    ...
                                                    <input type="hidden" class="hdnIsFinalDiscount" value="<%# Eval("IsFinalDiscount")%>" />
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:BoundField HeaderStyle-Width="250px" DataField="CompanyName" ItemStyle-CssClass="tdCompanyName" HeaderText="Company" />
                                            <asp:BoundField DataField="PICMemberName" HeaderText="PIC" ItemStyle-CssClass="tdContactPersonName" />
                                            <asp:BoundField HeaderStyle-Width="120px" DataField="PaymentDateInString" ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center" HeaderText="Payment Date" />
                                            <asp:BoundField HeaderStyle-Width="80px" DataField="cfPrice" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Center" HeaderText="Price" />
                                            <asp:BoundField HeaderStyle-Width="80px" DataField="cfDiscountAmount" ItemStyle-HorizontalAlign="Right" HeaderStyle-HorizontalAlign="Center" HeaderText="Discount" />
                                        </Columns>
                                        <EmptyDataTemplate>
