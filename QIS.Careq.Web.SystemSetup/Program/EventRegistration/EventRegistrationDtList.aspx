...
<asp:Content ID="Content2" ContentPlaceHolderID="plhCustomButtonToolbar" runat="server">
    ...
    <li id="btnSendMailCompany" runat="server"><img src='<%=ResolveUrl("~/Libs/Images/Icon/tbsendmail.png")%>' alt="" /><br style="clear:both"/><div><%=GetLabel("Send Mail 2")%></div></li>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhList" runat="server">
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnSendMailCompany.ClientID %>').click(function () {
                getCheckedMember();
                if ($('#<%=hdnSelectedMember.ClientID %>').val() == '')
                    alert('Please Select Member First');
                else {
                    var id = $('#<%=hdnEventID.ClientID %>').val() + '|' + $('#<%=hdnSelectedMember.ClientID %>').val();
                    var url = ResolveUrl("~/Program/EventRegistration/EventConfirmationSendEmailCompanyCtl.ascx");
                    openUserControlPopup(url, id, 'Send Email', 700, 500);
                }
            });
        });
